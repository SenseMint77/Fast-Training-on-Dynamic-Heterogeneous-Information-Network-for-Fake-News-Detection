...
class SparseGraph:
    ...
    label
        scalar, represents the respective graph's label
    node_names
        Names of nodes (as strings). Shape [num_nodes]
    attr_names
        Names of the attributes (as strings). Shape [num_attr]
    class_names
        Names of the class label (as strings). Shape [num_classes]
    metadata
    ...
    def __init__(
        ...
        if attr_matrix is not None:
            ...
            if attr_matrix.shape[0] != adj_matrix.shape[0]:
                raise ValueError(
                    "Dimensions of the adjacency {} and attribute matrices {} don't agree.".format(adj_matrix.shape, attr_matrix.shape))

        if label is None:
            raise ValueError("Label cannot be null for supervised learning.")
    ...
    def to_undirected(self) -> 'SparseGraph':
        ...
        if len(dup_ridx) > 0 and not np.allclose(self.adj_matrix[dup_idx], self.adj_matrix[dup_idx[::-1]]):
            raise ValueError(
                "Adjacency matrix weights of opposing edges differ.")

        # Create symmetric matrix
        new_adj_matrix = self.adj_matrix + self.adj_matrix.T
        if len(dup_ridx) > 0:
            new_adj_matrix[dup_idx] = (
                new_adj_matrix[dup_idx] - self.adj_matrix[dup_idx]).A1
    ...
    def unpack(self) -> Tuple[sp.csr_matrix,
        ...
        """
        return self.adj_matrix, self.attr_matrix, self.label
    ...
    def from_flat_dict(data_dict: Dict[str, Any]) -> 'SparseGraph':
        ...
        for key in data_dict.keys():
            if key.endswith('_data') or key.endswith('.data'):
                ...
                del_entries.extend(
                    [mat_data, mat_indices, mat_indptr, mat_shape])
        ...
        unknown_keys = [key for key in init_dict.keys(
        ) if key not in sparse_graph_properties]
